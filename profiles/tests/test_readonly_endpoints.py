...
def test_check_disallowed_method_util(user_api_client):
    ...
    only_put_method = 'put'
    check_disallowed_methods(user_api_client, CONCEPTS_OF_INTEREST_LIST, only_put_method)

    # test multiple methods for multiple urls
    interest_concepts_urls = [
        get_interest_concept_url(1),
        get_interest_concept_url(2)
    ]
    other_disallowed_methods = ('patch', 'delete')
    check_disallowed_methods(user_api_client, interest_concepts_urls, other_disallowed_methods)


def test_concepts_of_interest_readonly(user_api_client):
    list_disallowed_methods = ('post', 'put', 'patch', 'delete')
    check_disallowed_methods(user_api_client, CONCEPTS_OF_INTEREST_LIST, list_disallowed_methods)
    check_disallowed_methods(user_api_client, get_interest_concept_url(), list_disallowed_methods)


def test_geodivisions_readonly(user_api_client):
    list_disallowed_methods = ('post', 'put', 'patch', 'delete')
    check_disallowed_methods(user_api_client, GEO_DIVISION_LIST, list_disallowed_methods)
    check_disallowed_methods(user_api_client, get_geodivision_url(), list_disallowed_methods)
